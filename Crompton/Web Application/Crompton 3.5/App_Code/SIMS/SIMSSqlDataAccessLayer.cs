...
/// <summary>
/// Summary description for SIMSSqlDataAccessLayer
/// </summary>
/// 
public sealed class SIMSSqlDataAccessLayer
{
    ...
    public SIMSSqlDataAccessLayer()
    {
        connectionString = ConfigurationManager.ConnectionStrings["SIMSconnStr"].ToString();
    }

    public SIMSSqlDataAccessLayer(string SbsDb)
    {
        connectionString = ConfigurationManager.ConnectionStrings[SbsDb].ToString();
    }
    /// <summary>
...
/////////Added By Pravesh//////////////
    ...
    public int ExecuteNonQuery(SqlConnection connection, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
    {
        ...
        // Create a command and prepare it for execution
        SqlCommand cmd = new SqlCommand();
        //cmd.CommandTimeout = 360; // BP 19sept // comment 20 sept already assigned in Prepare cmd
        bool mustCloseConnection = false;
        PrepareCommand(cmd, connection, (SqlTransaction)null, commandType, commandText, commandParameters, out mustCloseConnection);
    ...
    /// <summary>
    /// This enum is used to indicate whether the connection was provided by the caller, or created by SIMSSqlDataAccessLayer, so that
    /// we can set the appropriate CommandBehavior when calling ExecuteReader()
    /// </summary>
    private enum SqlConnectionOwnership
    {
        /// <summary>Connection is owned and managed by SIMSSqlDataAccessLayer</summary>
        Internal,
    ...
    /// <param name="commandText">The stored procedure name or T-SQL command</param>
    /// <param name="commandParameters">An array of SqlParameters to be associated with the command or 'null' if no parameters are required</param>
    /// <param name="connectionOwnership">Indicates whether the connection parameter was provided by the caller, or created by SIMSSqlDataAccessLayer</param>
    ...
    private SqlDataReader ExecuteReader(SqlConnection connection, SqlTransaction transaction, CommandType commandType, string commandText, SqlParameter[] commandParameters, SqlConnectionOwnership connectionOwnership)
    {
        ...
        try
        {
            ...
            //connection.Close();   // Added by Mukesh as on 9/Jun/2015 Not working on service contractor page
            return dataReader;
        }
        catch
        {
