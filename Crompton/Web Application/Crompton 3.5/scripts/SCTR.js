...
//////////////////////////Logic change by Priyam 02/02/2016 for all divisions//////////////////////////////
        function ValidateBatchCodeWithLogicChanged(oSrc, args) {
           // alert("New Method Start");
            debugger;
            ...
            var unit;
             ...
             productType = inputProdSer.substring(0, 1);// Cheking local or imported product --Priyam
             unit=     inputProdSer.substring(4, 5);


            ///Check for the unit which lies between 0-4 --Priyam
             if ((unit < 6) && (unit>0)) 
             {


                inputBatchYear = inputProdSer.substring(5, 6);//(0,1)
                inputBatchMonth = inputProdSer.substring(6, 7);//(1,2)
                inputC3code = inputProdSer.substring(1, 4); // (2,5)
                ...
                if (ddlPline.selectedIndex > 0) {
                    ...
                    if ((productType == 'I' || productType == 'L' || productType == 'M') && (isNumeric(unit))) {
                      ...
                      //  alert(ProductType);
                       ...
                       // alert(inputC3code);
                       // alert(arrayValidCode);

                        var bolValidCode = new Boolean(false);
                        for (vcode in arrayValidCode) {
                            var CodeToCompare = arrayValidCode[vcode];
                            //alert(inputC3code);
                            //alert(CodeToCompare.substring(0, 3));
                            if (CodeToCompare.substring(0, 3) == inputC3code) {
                                var arrayInputCode = CodeToCompare.split('_');
                                //alert(arrayInputCode);
                                if (arrayInputCode[1] != inputPline) {
                                   // alert(arrayInputCode[1], inputPline);
                                    BatchNo.value = "";
                                    bolValidCode = false;
                                }

                                else {
                                    BatchNo.value = inputProdSer.substring(5, 7);//(5,7)

                                    /////As the location vendor master has the same code but unit is diffrent--Priyam
                                    if (inputC3code == 'MF0') {
                                        if (unit == 1) {
                                            txtMfgUnit.value = 'Bethora';
                                        }
                                        else if (unit == 2) {
                                            txtMfgUnit.value = 'Kundiam';

                                        }
                                        else if (unit == 3) {
                                            txtMfgUnit.value = 'Baddi Unit1';

                                        }
										
										else if (unit == 5) {
                                            txtMfgUnit.value = 'Baddi Unit3';

                                        }
                                        else {
                                            txtMfgUnit.value = 'Baddi Unit2';
                        ...
                        if (bolValidCode == false) {
                            args.IsValid = false;
                            alert("Invalid ProdLine / ProdSrNo1");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        function validateBatchCode(oSrc, args) {
            ...
            ///////////////////Filling varraible values and checking null condition//////////////////////////////
            ...
            ////////////////////////Set the value of Year and month varraible///////////////////
          
            if (inputProdSer.length > 11) {
               // alert("if");
                inputBatchYear = inputProdSer.substring(5, 6); ////(0,1)
                inputBatchMonth = inputProdSer.substring(6, 7);///////(1,2)
                ...
                inputBatchYear = inputProdSer.substring(0, 1); ////(0,1)
                inputBatchMonth = inputProdSer.substring(1,2);///////(1,2)
            ...
            // Second condition in if is added by Ashok for applying new logic to fan, pump division
          ...
          
            {
                ...
                if (inputProdSer.length > 11) {
                    ...
                    //alert("In New Method");
                  //  debugger;
                    ValidateBatchCodeWithLogicChanged(oSrc, args);//////Priyam New logic
                 ...
                 txtMfgUnit.style.display = 'none';
                 txtMfgUnit.disabled = true;
                ...
                if ((productDiv == 'APPLIANCES' || productDiv == 'APPLIANCE')) {
                    ...
                    {     
                         debugger;
                         ValidateBatchCodeWithLogicChanged(oSrc, args);///by @VT
                       if(args.IsValid == true)
                       {
                       
                         inputProdSer = inputProdSer.substring(5, 7);
                         inputBatchYear = inputProdSer.substring(0, 1);
                         inputBatchMonth = inputProdSer.substring(1, 2);
                       }
                       else
               ...
               //////////////////////////////New Change Priyam 27/06/17////
			   
			   if((strString.substring(0, 1)=='C')&&(strString.length > 11))
               {
                 inputProdSer="";
                 inputBatchYear ="";
                 inputBatchMonth="";
                ...
                if (bolBatch == false) {
                    ...
                    if (productDiv.toUpperCase() == 'Fans'.toUpperCase()) {
                        ...
                        //alert("fan");
                        inputProductMfg = inputProdSer1.substring(2,3); //(2,3)

                       // alert(inputProductMfg);
        ...
        function OpenConfirmDiv() {
	     ...
	     //debugger;
	     var ComplainDate = document.getElementById("ctl00_MainConHolder_lblComplainDate").innerHTML;
	     var InvoiceDate = document.getElementById("ctl00_MainConHolder_txtInvoiceDate").value;
            
         var productunit = document.getElementById("ctl00_MainConHolder_lblUnit").innerHTML; 
               
	     var isValid = false;
	     var date1 = new Date(ComplainDate);
	     var date2 = new Date(InvoiceDate);
	     if (date1 > date2) 
	     {
	         isValid = true;
	         var days = Math.floor((Date.UTC(date1.getFullYear(), date1.getMonth(), date1.getDate()) - Date.UTC(date2.getFullYear(), date2.getMonth(), date2.getDate())) / (1000 * 60 * 60 * 24));
	         if(productunit.toLowerCase()==="fans")
	         {  
	         if (days <= 730) 
	         {
	             // set yes 
	             document.getElementById("ctl00_MainConHolder_rdbWarranty_0").checked = true;
	         } 
	         else
	          {
	             // set warrenty false 
	             document.getElementById("ctl00_MainConHolder_rdbWarranty_1").checked = true;
	         }
	         }
	     }
	      else 
	      {
	         ...
	         alert("Invoice date should be older than complaint date");
	         //clear control 
