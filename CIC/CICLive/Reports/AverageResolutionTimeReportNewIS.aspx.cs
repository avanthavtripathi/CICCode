...
public partial class Reports_AverageResolutionTimeReportNewIS : System.Web.UI.Page
{
    ...
    {
        DataSet ds = objSql.ExecuteDataset(CommandType.StoredProcedure, "usp_AvgResolutionTimeReportIS");
    ...
    private void FindheaderTextAndHide()
    {
            ...
            {
                if (listDict.ContainsKey(gvReport.Columns[i].HeaderText) && listDict.ContainsValue("IS"))
                    gvReport.Columns[i].Visible = true;
                
                else if (gvReport.Columns[i].HeaderText=="IS")
                    gvReport.Columns[i].Visible = true;
